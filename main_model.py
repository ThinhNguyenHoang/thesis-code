...
class MainModel(keras.Model):
    ...
    def compile(self, optimizer, loss_fn=None, **kwargs):
        super().compile(loss=kwargs['loss'])
    ...
    def load_encoder(self, config: Config):
        ...
        encoder = tf.keras.Model(inputs, outputs, name='evaluator_feature_extractor')
    ...
    def init_decoders(self, config: Config):
        decoders = [self.load_decoder(config, pool_dimension, i) for i, pool_dimension in enumerate(self.pool_dimensions)]
        return decoders

    # input_dimension: BxHxWxC
    def load_decoder(self, config:Config, input_dimension, layer_id):
        decoder_arch = config.decoder_arch
        if config.debug:
            debug.debug_print(f'load_decoder_arch::input_dimenstion={input_dimension}')
        decoder = self.cflow(config, input_dimension, layer_id)
        return decoder
    # input_dimension: BxHxWxC
    def cflow(self, config: Config, input_dimension, pool_layer_id):
    ...
    # Data should of shape (B, D) with D is struture of shape (image_input, label, ground_truth_mask)
    # ground_truth_label can be none
    # @tf.function
    def call(self, data):
        # What to return ???
        sample_height = 0
        ...
        ground_truth_labels = []
        ground_truth_masks = []
        log_props_at_scales = [list() for i in range(self.config.pool_layers)]

        # inspect data
        ground_truth_provided = False # data[0]['ground_truth_mask']

        if self.config.verbose:
            print("CALL::num_of_multiscale_features:", self.config.pool_layers, "INPUT shape:", data.shape)

        start = time.time()
        # extract saliency map
        saliency_map = self.saliency_detector(data, Image.BICUBIC)[0] # BxHxWx1
        feature_maps = self.encoder(data)
        for pool_layers_idx, feature_map in enumerate(feature_maps):
            ...
            decoder = self.decoders[pool_layers_idx]
            # FIBER PROCESSING
            ...
            if ground_truth_provided:
                ...
                ground_truth_mask_patch = tf.reshape(ground_truth_mask, [num_features_vecs, 1])
            #
            N = self.config.N
            ...
            for unit in range(UNIT):
                ...
                log_prob = decoder_log_prob / depth # [240]
                # print("CAL::", log_props_at_scales, log_props_at_scales[pool_layers_idx] ,"adding", log_prob, "with:", log_prob.shape)
                log_props_at_scales[pool_layers_idx] = tf.concat([log_props_at_scales[pool_layers_idx],tf.stop_gradient(log_prob)], 0)
                # print("POSCAL::", log_props_at_scales, "adding", log_prob, "with:", log_prob.shape)
        # stop = time.time()
        # fps = len(data) / (stop - start)
        # if self.config.verbose:
        #     print(f"Call::DEBUG::Processed with fps {fps}")
        # RETURN
        # return sample_height, sample_width, images, ground_truth_labels, ground_truth_masks, log_props_at_scales
        return log_props_at_scales
    
    def train_step(self, data):
        feature_maps = self.encoder(data, training=False)
        print("TRAIN_STEP::num_of_multiscale_features:", len(feature_maps))
    ...
    def test_step(self, data):
        if self.config.verbose:
            print("TEST::Computing loss and scores on test set:")
